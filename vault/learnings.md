...
- **Positioning claims must be checked against the repo, not taken from the brief.** The ask was to advertise "LLM features shipped to production (iReserb, Alfardan, PCCMobile)" — but those projects' own descriptions are Firebase/Maps, UIKit customer-service chat, and Kotlin herd management. No LLM anywhere. The real, provable differentiator is AI-*accelerated delivery* (70% debug-time cut). Shipping the original claim would have been a credibility trap for a client who clicks through.
